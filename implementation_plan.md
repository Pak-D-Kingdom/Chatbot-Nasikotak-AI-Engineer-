...
6. **Embedding Model Size** — BGE-M3 cukup besar (~2.3GB). Untuk MVP, saya rekomendasikan opsi **BGE-M3** (multilingual, lebih akurat) atau **all-MiniLM-L6-v2** (lebih ringan, ~80MB) dengan trade-off pada akurasi bahasa Indonesia.
...
1. **Embedding Model**: BGE-M3 (lebih akurat untuk Bahasa Indonesia, tapi butuh ~2.3GB RAM) vs all-MiniLM-L6-v2 (lebih ringan, mungkin kurang akurat untuk bahasa Indonesia)? PRD menyebutkan BGE-M3.
...
| 3.3 | BGE-M3 embedding generation |
